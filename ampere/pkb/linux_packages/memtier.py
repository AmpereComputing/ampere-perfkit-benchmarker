...
PACKAGE_NAME = "ampere_memtier"

GIT_REPO = "https://github.com/RedisLabs/memtier_benchmark"
GIT_TAG = "d5ac1f4"
MEMTIER_DIR = "%s/memtier_benchmark" % download_utils.INSTALL_DIR
APT_PACKAGES = (
    "build-essential autoconf automake libpcre3-dev "
    "libevent-dev pkg-config zlib1g-dev libssl-dev"
)
YUM_PACKAGES = "zlib-devel pcre-devel libmemcached-devel libevent-devel openssl-devel"
MEMTIER_RESULTS = "/tmp/memtier_results"

_LOAD_NUM_PIPELINES = 155  # Arbitrarily high for loading
_WRITE_ONLY = "1:0"
CPU_TOLERANCE = 0.05
WARM_UP_SECONDS = 360
...
flag_util.DEFINE_integerlist(
    f"{PACKAGE_NAME}_threads", [4], "Number of threads. Defaults to 4."
)
MEMTIER_RATIO = flags.DEFINE_string(
    f"{PACKAGE_NAME}_ratio", "1:9", "Set:Get ratio. Defaults to 1:9 Sets:Gets."
)
MEMTIER_DATA_SIZE = flags.DEFINE_integer(
    f"{PACKAGE_NAME}_data_size", 32, "Object data size. Defaults to 32 bytes."
)
MEMTIER_DATA_SIZE_LIST = flags.DEFINE_string(
    f"{PACKAGE_NAME}_data_size_list",
    None,
    "Used in place of ampere_memtier_data_size. "
    "Use sizes from weight list (size1:weight1,..sizeN:weightN)",
)
MEMTIER_LOAD_KEY_PATTERN = flags.DEFINE_string(
    f"{PACKAGE_NAME}_load_key_pattern",
    None,
    "Same as ampere_memtier_key_pattern, used during load phase. "
    "Defaults to None, use S:S for baremetal testing.",
)
MEMTIER_KEY_PATTERN = flags.DEFINE_string(
    f"{PACKAGE_NAME}_key_pattern",
    "R:R",
...
MEMTIER_LOAD_KEY_MAXIMUM = flags.DEFINE_integer(
    f"{PACKAGE_NAME}_load_key_maximum",
    None,
    "Key ID maximum value to load. "
...
MEMTIER_KEY_MAXIMUM = flags.DEFINE_integer(
    f"{PACKAGE_NAME}_key_maximum",
    10000000,
    "Key ID maximum value. The range of keys "
...
MEMTIER_CPU_DURATION = flags.DEFINE_integer(
    f"{PACKAGE_NAME}_cpu_duration",
    300,
    "Number of seconds worth of data taken "
...
flag_util.DEFINE_integerlist(
    f"{PACKAGE_NAME}_pipeline",
    [1],
    "Number of pipelines to use for memtier. Defaults to 1, " "i.e. no pipelining.",
)
MEMTIER_CLUSTER_MODE = flags.DEFINE_bool(
    f"{PACKAGE_NAME}_cluster_mode", False, "Passthrough for --cluster-mode flag"
)
MEMTIER_TIME_SERIES = flags.DEFINE_bool(
    f"{PACKAGE_NAME}_time_series",
    False,
    "Include per second time series output "
    "for ops and max latency. This greatly increase the number of samples.",
)
_NUMA_CORES = flags.DEFINE_list(
    f"{PACKAGE_NAME}_numa_cores_list",
    [],
    f"Used to pin each memtier process to a core with numactl during the run phase. "
...
def BuildMemtierCommand(
    ...
    # Build the command with or without numa pinning
    if numa_core:
        pre = f"numactl -C {numa_core}"
        cmd = [pre, "memtier_benchmark"]
    else:
...
def Load(
    ...
    data_size = MEMTIER_DATA_SIZE.value if not MEMTIER_DATA_SIZE_LIST.value else None
    cmd = BuildMemtierCommand(
        ...
        random_data=True,
        key_pattern=MEMTIER_LOAD_KEY_PATTERN.value,
        requests=MEMTIER_REQUESTS.value,
...
def RunOverAllThreadsPipelinesAndClients(
    ...
    samples = []
    for pipeline in FLAGS.ampere_memtier_pipeline:
        for client_thread in FLAGS.ampere_memtier_threads:
            for client in FLAGS.ampere_memtier_clients:
                logging.info(
...
def MeasureLatencyCappedThroughput(
    client_vm, server_ip: str, server_port: str, password: Optional[str] = None
) -> List[sample.Sample]:
...
def RunGetLatencyAtCpu(cloud_instance, client_vms):
    ...
    while lower_bound < upper_bound:
        ...
        cpu_percent = cloud_instance.MeasureCpuUtilization(MEMTIER_CPU_DURATION.value)
...
def _GetSingleThreadedLatency(
    client_vm, server_ip: str, server_port: str, password: str
) -> "MemtierResult":
...
def _get_numa_cores():
    """Helper function returns a list of all numa cores specified by flag
    e.g. ['0', '1', '2', ...]
    """
    all_cores = []
...
def _Run(
    ...
    requests = MEMTIER_REQUESTS.value if MEMTIER_RUN_DURATION.value is None else None
    test_time = (
    ...
    # TODO: debug
    logging.debug(f"MEMTIER RUN: {cmd}")
    vm.RobustRemoteCommand(cmd)

    output_path = os.path.join(vm_util.GetTempDir(), f"memtier_results_{server_port}")
...
class MemtierResult:
    ...
    @classmethod
    def Parse(
        cls, memtier_results: Text, time_series_json: Optional[Text]
    ) -> "MemtierResult":
        ...
        return cls(
            ...
            get_ops_per_sec=aggregated_result.get_ops_per_sec,
            hits_per_sec=aggregated_result.hits_per_sec,
    ...
    def GetSamples(self, metadata: Dict[str, Any]) -> List[sample.Sample]:
        ...
        samples = [
            ...
            sample.Sample("Hits/s", self.hits_per_sec, "hits/s", metadata),
            sample.Sample("Get Ops/s", self.get_ops_per_sec, "get ops/s", metadata),
        ]
        for name, histogram in [
            ...
            samples.append(sample.Sample(f"{name} latency histogram", 0, "", hist_meta))
        ...
        for interval, latency in self.max_latency_time_series:
            ...
            samples.append(
                sample.Sample(
                    "Max Latency Time Series", latency, "ms", time_series_meta
                )
            )
        return samples


def _ParseHistogram(memtier_results: Text) -> Tuple[MemtierHistogram, MemtierHistogram]:
...
def _ParseTotalThroughputAndLatency(memtier_results: Text) -> "MemtierAggregateResult":
    ...
    for raw_line in memtier_results.splitlines():
        ...
        if re.match(r"^Totals", line):
            ...
            return MemtierAggregateResult(
                ...
                hits_per_sec=_FetchStat("Hits/sec"),
                get_ops_per_sec=_FetchGetStat("Ops/sec"),
            )
