...
def download_file(
    url: str, *, filename: str = None, md5: str = None, timeout: float = None
):
    ...
    if md5 and md5 != checksum:
        raise Exception(
            f"{filename} checksum of {checksum} did not match expected {md5}"
        )


def download(
    url: str,
    vms: List[BaseVirtualMachine],
    *,
    cache_dir: str = None,
    md5: str = None,
    sha256: str = None,
    ...
    Args:
        ...
        cache_dir: Optionally select a directory on the PKB host
        to store the gsutil long-term and cache.
    ...
    with TemporaryDirectory() as scratch_dir:
        ...
        target_vms = [
            vm.RemoteCommand(f"ls {dst_file}", ignore_failure=True) for vm in vms
        ]
        ...
        if md5:
            ...
            if check != md5:
                raise errors.Setup.InvalidSetupError(
                    f"MD5 sum {check} didn't match expected: {md5}"
                )
        if sha256:
            check = sha256sum(src_file)
            if check != sha256:
                raise errors.Setup.InvalidSetupError(
                    f"SHA256 sum {check} didn't match expected: {sha256}"
                )
...
def gsutil(
    ...
    *,
    ...
    Args:
        uri: gs://your-file
        vms: List of target Machines
        cache_dir: Optionally select a directory on the PKB host to
         store the gsutil long-term and cache.
                   Defaults to temporary directory.
        md5: Optionally check the file against an md5 checksum
        sha256: Optionally check the file against an sha256 checksum
        dst: Optionally select target directory. Defaults to /opt/pkb-cache
...
def send_resource_to_vm(vm, path):
    ...
    if not data.ResourceExists(local_path):
        try:
            ...
            logging.debug(
                f"Successfully copied resources from {path} to {DOWNLOAD_LOC}: {stdout.decode()}"
            )
            ...
            if e.returncode == 127:  # gsutil not found
                ...
                raise errors.Setup.InvalidSetupError(
                    f"Copying resources from {path} to {DOWNLOAD_LOC} "
    ...
    _, _, retcode = vm.RemoteCommandWithReturnCode(
        f"test -f {deploy_file}", ignore_failure=True
    )
    ...
    if not file_exists:
        ...
        if not os.path.exists(LOCAL_FILE):
            raise errors.Setup.InvalidSetupError(
                f"Resource from {path} does not exist: {LOCAL_FILE}"
            )
        vm.RemoteCommand(f"sudo mkdir -p {CACHE_DIR}")
        vm.RemoteCommand(f"sudo chown -R $USER:$USER {CACHE_DIR}")
    ...
    if file_exists:
        return deploy_file
    raise errors.Setup.InvalidSetupError(
        f"Failed to copy resource from {path} to VM at path: {deploy_file}"
    )

def _get_package_from_git_url(git_url, git_branch, vm, recursive_submodule=False):
    add_recursive_submodule = ""
    if recursive_submodule:
        add_recursive_submodule = " --recurse-submodules "
    vm.RemoteCommand(f"cd {INSTALL_DIR}; git clone {add_recursive_submodule} --branch {git_branch} {git_url}")
