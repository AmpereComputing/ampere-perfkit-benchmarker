...
from ampere.pkb import systems, os_types
from ampere.pkb.providers.oci import util, oci_disk, oci_network
...
class OciVmSpec(virtual_machine.BaseVmSpec):
    ...
    def _ApplyFlags(cls, config_values, flag_values):
        ...
        if flag_values["oci_availability_domain"].present:
            config_values["oci_availability_domain"] = (
                flag_values.oci_availability_domain
            )
...
class OciVirtualMachine(virtual_machine.BaseVirtualMachine):
    ...
    def _WaitForInstanceStatus(self, status_list):
        ...
        logging.info(
            "Waits until the instance's status is one of statuses: %s", status_list
        )
    ...
    def _Create(self):
        ...
        if self.fault_domain is None:
            fd_list = util.GetFaultDomainFromAvailabilityDomain(
                self.availability_domain, self.profile
            )
            self.fault_domain = fd_list[0]

        if self.image is not None:
            oci_image, oci_os_name, oci_os_version = util.GetOciImageIdFromName(
                self.image, self.machine_type, self.profile
            )
        else:
            oci_os_name = util.GetOsFromImageFamily(self.DEFAULT_IMAGE_FAMILY)
            oci_os_version = util.GetOsVersionFromOs(
                self.DEFAULT_IMAGE_PROJECT, oci_os_name
            )
            oci_image = util.GetOciImageIdFromImage(
                oci_os_name, oci_os_version, self.machine_type, self.profile
            )
        self.image = oci_image

        shape_config = '\'{"memoryInGBs":%s,"ocpus":%s}\'' % (
            self.compute_memory,
            self.compute_units,
        )
    ...
    def CreateScratchDisk(self, disk_spec_id, disk_spec):
        ...
        data_disk = oci_disk.OciDisk(
            disk_spec, self.profile, self.name, self.availability_domain, disk_number
        )
...
class Ubuntu2204BasedOCIVirtualMachine(
    OciVirtualMachine, linux_virtual_machine.Ubuntu2204Mixin
):
    DEFAULT_IMAGE_FAMILY = "ubuntu-os-cloud"
    DEFAULT_IMAGE_PROJECT = "ubuntu-2204-lts"


class Ubuntu2004BasedOCIVirtualMachine(
    OciVirtualMachine, linux_virtual_machine.Ubuntu2004Mixin
):
    DEFAULT_IMAGE_FAMILY = "ubuntu-os-cloud"
    DEFAULT_IMAGE_PROJECT = "ubuntu-2004-lts"


class Oracle9BasedVirtualMachine(OciVirtualMachine, systems.oracle9.Oracle9Mixin):
    DEFAULT_IMAGE_FAMILY = "Oracle Linux"
    DEFAULT_IMAGE_PROJECT = "9"


class Oracle8BasedVirtualMachine(OciVirtualMachine, systems.oracle8.Oracle8Mixin):
    DEFAULT_IMAGE_FAMILY = "Oracle Linux"
