...
"""
Module containing functions to run dlrm and resnet50
"""
...
def check_threads_validity(benchmark_name: str):
    """
...
class PytorchRunModel:
    """
    Run DLRM/Resnet model
    """
    ...
    def __init__(self, expt_details: dict):
        ...
        self.num_available_threads = len(
            parse_threads_range(FLAGS[f"{self.benchmark}_threads_range"].value)
        )
    ...
    def run_pytorch_model(
        ...
        Returns:
            None
        """
        # set common flags for Dlrm/Resnet run
        cmd_dict_common = PytorchRunModel.common_flags(self)
        num_available_threads = cmd_dict_common["num_available_threads"]
        # call warm up function
        PytorchRunModel.warm_up_pytorch_model(self, cmd_dict_common)
        for batch_size in sorted(batch_sizes_list):
            for num_threads in sorted(threads_per_process_list, reverse=True):
                ...
                if f"{self.benchmark}_number_of_models" in FLAGS:
                    if FLAGS[f"{self.benchmark}_number_of_models"].value > 0:
                        num_processes = FLAGS[f"{self.benchmark}_number_of_models"].value
                ...
                PytorchRunModel.run_pytorch_command(self, cmd_dict, cmd_dict_common)

    def warm_up_pytorch_model(self, cmd_dict_common):
        """warm up function to run dlrm/resnet"""
        metadata = {}
        cmd_dict = {
            "batch_size": 1,
            "num_threads": 1,
    ...
    def common_flags(self):
        ...
        num_available_threads = parse_threads_range(threads_range)
        source_cmd = ""
        if precision == "fp16":
            source_cmd = (
    ...
    def run_pytorch_command(self, cmd_dict, cmd_dict_common):
        ...
        model_path = FLAGS[f"{self.benchmark}_runner_path"].value
        cmd = (
            f"{cmd_dict_common['source_cmd']} "
            f"python3 {volume_mountpoints[0]} -p {str(cmd_dict_common['precision'])} "
